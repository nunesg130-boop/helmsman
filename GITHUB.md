...
```text
%USERPROFILE%\Downloads\helmsman-v1.3.3\helmsman-1.3.3-deployment-assets
```
...
```powershell
$Assets = "$env:USERPROFILE\Downloads\helmsman-v1.3.3\helmsman-1.3.3-deployment-assets"
$DeployHost = "deploy-user@helmsman-host.example"
ssh $DeployHost "mkdir -p /srv/apps/helmsman/releases/v1.3.3"
scp "$Assets\compose.yaml" "${DeployHost}:/srv/apps/helmsman/releases/v1.3.3/"
scp "$Assets\container.env.example" "${DeployHost}:/srv/apps/helmsman/releases/v1.3.3/"
scp "$Assets\SHA256SUMS" "${DeployHost}:/srv/apps/helmsman/releases/v1.3.3/"
ssh $DeployHost
```
...
```sh
set -euo pipefail
cd /srv/apps/helmsman/releases/v1.3.3
...
- back up the configured installation's `compose.yaml` and, when present,
  `.env` as `compose.yaml.before-1.3.3` and
  `.env.before-1.3.3`;
