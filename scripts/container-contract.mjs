...
record(
  missing.length === 0,
  "Helmsman v1.3.3 includes its unified media model, bounded fixed actions, Proxmox and Portainer infrastructure monitors, encrypted store, persistent cache, Obsidian Glass operations UI, logging, and deployment contracts",
  missing.join(", ")
);
...
if (existsSync(join(root, "Dockerfile"))) {
  ...
  record(
    /^ARG HELMSMAN_VERSION=1\.3\.3$/mu.test(dockerfile)
    ...
    "image metadata carries the Helmsman v1.3.3 identity and license"
  );
...
if (existsSync(join(root, "server/broker.mjs"))) {
  ...
  record(
    /const DEFAULT_VERSION = "1\.3\.3"/u.test(broker)
      && /process\.env\.HELMSMAN_VERSION/u.test(broker)
      && /\^\[0-9A-Za-z\]\[0-9A-Za-z\.\+-\]\{0,63\}\$/u.test(broker),
    "runtime version follows the validated immutable v1.3.3 image metadata"
  );
}
...
if (existsSync(join(root, "server/index.mjs"))
  ...
  record(
    ...
    "the beta.2 access-key route is migration-only and v1.3.3 exposes no access-key rotation route or CLI"
  );
  record(
...
if (existsSync(join(root, "compose.yaml"))) {
  ...
  record(
    /^name:\s*helmsman\s*$/mu.test(compose)
      ...
      && compose.includes('${HELMSMAN_IMAGE:-ghcr.io/OWNER/REPOSITORY:1.3.3}')
...
if (existsSync(join(root, "compose.dev.yaml"))) {
  ...
  record(
    /^services:\s*\n\s{2}helmsman:\s*$/mu.test(developmentCompose)
      ...
      && /HELMSMAN_VERSION:\s*["']1\.3\.3["']/u.test(developmentCompose)
      && /HELMSMAN_REVISION:\s*["']local["']/u.test(developmentCompose)
      && /image:\s*["']helmsman:1\.3\.3["']/u.test(developmentCompose),
...
if (existsSync(join(root, "container.env.example"))) {
  ...
  record(
    assignments.includes("HELMSMAN_IMAGE=ghcr.io/OWNER/REPOSITORY:1.3.3")
...
if (existsSync(join(root, "manifest.webmanifest"))) {
  try {
    ...
    record(
      ...
      "the v1.3.3 installed-app manifest opens canonical Media Overview and retains dedicated local application icons"
    );
  } catch (error) {
    record(false, "the v1.3.3 installed-app manifest opens canonical Media Overview and retains dedicated local application icons", error.message);
  }
}
...
if (existsSync(join(root, "index.html"))) {
  ...
  record(
    /data-action="switch-workspace"\s+data-workspace="system"/u.test(shell)
    ...
    "desktop and compact navigation expose semantic Overview, Media, and Infrastructure workspace controls"
  );
  ...
  const sharedRouteCount = (route) => (
    shell.match(new RegExp(`<a(?=[^>]*data-route="${route}")(?![^>]*workspace-(?:system|media|infrastructure)-only)[^>]*>`, "gu")) || []
  ).length;
  ...
  record(
    ["overview", "discover", "library", "requests", "activity", "calendar", "health", "connections"]
      ...
      && workspaceRouteCount("system", "system") === 2
      && ["logs", "settings"].every((route) => sharedRouteCount(route) === 2)
      && /\[hidden\]\s*\{\s*display:\s*none\s*!important;\s*\}/u.test(shellStyles),
    "desktop and mobile navigation expose all three workspaces, gated infrastructure routes, and shared routes"
  );
  record(
    /data-action="toggle-sidebar"[^>]+aria-controls="sidebar-navigation"[^>]+aria-expanded="true"/u.test(shell)
      ...
      && /<div[^>]+id="monitor-summary"[^>]+aria-label="Current health summary"/u.test(shell)
      && /class="monitor-summary__metric" href="#\/connections"/u.test(shell)
      && /class="monitor-summary__metric" href="#\/health"/u.test(shell)
...
if (existsSync(join(root, "src/app-v5.js")) && existsSync(join(root, "src/ui/operations-views.js"))) {
  ...
  record(
    /const INFRASTRUCTURE_ROUTE_ALIASES\s*=\s*Object[.]freeze\(\{[\s\S]*?environments:\s*"proxmox"[\s\S]*?nodes:\s*"proxmox"/u.test(application)
      ...
      && /href="#\/connections"/u.test(application)
      && /href="#\/health"/u.test(application)
      && /href="#\/incidents"/u.test(application),
