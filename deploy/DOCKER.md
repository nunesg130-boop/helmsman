# Helmsman Docker deployment — v1.0.4
...
The GitHub Release for `v1.0.4` publishes three deployment assets:
...
cd /opt/helmsman
curl -fLO https://github.com/nunesg130-boop/helmsman/releases/download/v1.0.4/compose.yaml
curl -fLO https://github.com/nunesg130-boop/helmsman/releases/download/v1.0.4/container.env.example
curl -fLO https://github.com/nunesg130-boop/helmsman/releases/download/v1.0.4/SHA256SUMS
...
set -euo pipefail
cd /opt/helmsman
cp -- compose.yaml.before-1.0.4 compose.yaml
if [ -f .env.before-1.0.4 ]; then
  cp -- .env.before-1.0.4 .env
...
For v1.0.4, the workflow publishes Linux AMD64 and ARM64 images under the
...
- the GitHub Release is stable for v1.0.4;
