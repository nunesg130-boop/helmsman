# Helmsman Docker deployment — v1.3.0
...
v1.3.0 retains Media **Overview** as the canonical landing page and redirects legacy
...
The v1.3.0 Obsidian Glass overhaul changes presentation only. It applies the
...
The GitHub Release for `v1.3.0` publishes three deployment assets:
...
cd /opt/helmsman
curl -fLO https://github.com/nunesg130-boop/helmsman/releases/download/v1.3.0/compose.yaml
curl -fLO https://github.com/nunesg130-boop/helmsman/releases/download/v1.3.0/container.env.example
curl -fLO https://github.com/nunesg130-boop/helmsman/releases/download/v1.3.0/SHA256SUMS
...
set -euo pipefail
cd /opt/helmsman
cp -- compose.yaml.before-1.3.0 compose.yaml
if [ -f .env.before-1.3.0 ]; then
  cp -- .env.before-1.3.0 .env
...
For v1.3.0, the workflow publishes Linux AMD64 and ARM64 images under the
...
- the GitHub Release is stable for v1.3.0;
