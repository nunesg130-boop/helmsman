...
The supported Helmsman v1.0.1 deployment uses the GitHub Release
...
Pushing the Git tag `v1.0.1` runs the Node contracts and Linux AMD64/ARM64 smoke
...
When upgrading from v1.0.0-beta.2, its access key is a temporary migration credential only. Use it or an existing browser session to enroll the Jellyfin owner. Successful enrollment removes the verifier permanently and revokes all legacy sessions; v1.0.1 cannot create or rotate another browser key.
